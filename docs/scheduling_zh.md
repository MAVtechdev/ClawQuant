...
ClawQuant 的调度子系统：心跳、Cron 与投递的干净实现（双触发汇聚的本地 agent 常见模式）。
...
两条独立的触发线（cron timer + heartbeat interval）最终汇聚到同一个执行入口。共用 scheduler 天然支持唤醒合并 — cron 和心跳在同一个 250ms 窗口内只执行一次。
...
投递以 connector 为先：单租户场景下，用户最后在哪个渠道跟 bot 说话，bot 就在那个渠道回复。以后加新 connector 只需要 `registerConnector()` + `touchInteraction()`，不用改调度逻辑。
