...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Ensure the ClawQuant browser control service is running (see browser docs; legacy CLI: \`${formatCliCommand("openclaw gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the ClawQuant browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the ClawQuant browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
